defmodule MyMusic.Library.Album do
  ...
  def import_changeset(changes, attrs) do
    cond do
  ...
  def first_played_changeset(changes, attrs) do
    cond do
      is_map(attrs.first_played) &&
          Map.has_key?(attrs.first_played, :year) ->
        date = [
  ...
  def changeset(%Album{} = album, attrs) do
    ...
    cond do
      Map.has_key?(attrs, :first_played) ->
        first_played_changeset(changes, attrs)

      Map.has_key?(attrs, "first_played") ->
        import_changeset(changes, attrs)
